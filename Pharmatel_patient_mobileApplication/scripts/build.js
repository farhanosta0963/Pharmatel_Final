...
function getDeploymentOrigin() {
  const rawOrigin = process.env.DEPLOYMENT_ORIGIN;
  if (rawOrigin) {
    return rawOrigin.replace(/\/+$/, "");
  }

  if (process.env.REPLIT_INTERNAL_APP_DOMAIN) {
    return `https://${stripProtocol(process.env.REPLIT_INTERNAL_APP_DOMAIN)}`;
  }

  if (process.env.REPLIT_DEV_DOMAIN) {
    return `https://${stripProtocol(process.env.REPLIT_DEV_DOMAIN)}`;
  }

  if (process.env.EXPO_PUBLIC_DOMAIN) {
    return `https://${stripProtocol(process.env.EXPO_PUBLIC_DOMAIN)}`;
  }

  const fallbackPort = process.env.SERVE_PORT || process.env.PORT || "3000";
...
async function main() {
  ...
  const baseUrl = getDeploymentOrigin();
  ...
  console.log("Build complete! Serve from:", baseUrl);
